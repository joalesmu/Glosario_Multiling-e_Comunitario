...
const CONFIG = {
  ...
  COLS: {
    ...
    APPSHEET_ID:  'APPSHEET_APP_ID', // Celda clave para la replicabilidad
    MAIN_LANG:    'idioma_activo',
    API_TOKEN:    'API_TOKEN'
  },
  CACHE: { ENABLED: true, TTL: 21600, KEY: 'API_DATA_V6_2' }
};
...
/**
 * Detecta si el usuario usa coma o punto y coma en sus fórmulas.
 */
...
/**
 * Obtiene la configuración desde la hoja, incluyendo el ID dinámico de AppSheet.
 */
function getAppConfig(ss) {
  ...
  return {
    ...
    appsheetId: get(CONFIG.COLS.APPSHEET_ID), // ID vital para los links de grabación
...
/**
 * Abre el modal para seleccionar qué idiomas auditar en la Mesa de Trabajo.
 */
...
/**
 * El cerebro que cruza los IDs, busca faltantes y genera los hipervínculos dinámicos.
 */
function procesarReporteBack(seleccionados, opts) {
  const ss = SpreadsheetApp.getActiveSpreadsheet(); 
  const config = getAppConfig(ss);
  const sep = _getSeparator(); // Detección regional dinámica
  ...
  palabras.forEach(p => {
    ...
    seleccionados.forEach(sel => {
      ...
      if (idLang === idIdiomaBase) return; // No auditar el idioma base contra sí mismo
      ...
      if (tarea !== "") {
        // CONSTRUCCIÓN DEL LINK DINÁMICO
        // Si falta traducción, abrimos el formulario de Traducciones. 
        // Si existe pero falta audio, abrimos el formulario para editar esa fila.
        const view = (tarea === "📝 FALTA TRADUCCION") ? "Traducciones_Form" : "Traducciones_Form";
        const rowId = (tRow) ? tRow.id_traduccion : "";
        
        // Fórmula HYPERLINK dinámica adaptada al lenguaje del Sheet
        ...
        reporte.push([
          ...
          nomLang, 
          tarea, 
          "", // Espacio para INPUT manual si se prefiere el Sheet
          "", // Espacio para NOTA
          "PENDIENTE",
          formulaLink // La columna de acción dinámica
        ]);
  ...
  // Formato para celdas de entrada manual
...
function uiGenerarDashboardAdmin() {
  ...
  palabras.forEach(p => {
    ...
    // Link dinámico al detalle de la palabra en AppSheet
...
/**
 * Genera una lista limpia de palabras pendientes de imagen para el diseñador.
 */
function servicioExportarListaDisenador() {
  ...
  for (let i = 1; i < data.length; i++) {
    // Si la columna 4 (Estado Imagen) dice PENDIENTE
    if (String(data[i][3]).includes("PENDIENTE")) {
      lista.push([data[i][1]]); // Nombre del concepto
    }
  }
...
/**
 * Escanea Drive y genera un diccionario con/sin extensiones.
 */
function _obtenerDiccionarioDrive(folderId) {
  ...
  try {
    ...
    while (archivos.hasNext()) {
      ...
      diccionario.add(nom);               // perro.mp3
      diccionario.add(nom.split('.')[0]); // perro
    }
...
/**
 * Cruza ID_PALABRA con la hoja TRADUCCIONES para obtener el texto en el idioma base.
 */
...
function servicioLimpiarCache() { CacheService.getScriptCache().remove(CONFIG.CACHE.KEY); return true; }
